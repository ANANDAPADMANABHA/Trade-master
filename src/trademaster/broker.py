...
class AngelOneClient():
    ...
    def _load_instrument_list(self) -> None:
        ...
        if self.instrument_list is None:
            ...
            self.instrument_list: List[Dict[str, Union[str, int]]] = json.loads(
                response.read())
    ...
    def get_ltp(self, instrument_list: List[Dict[str, Union[str, int]]], ticker: str, exchange: str = "NSE") -> Optional[float]:
    ...
    def place_robo_order(self, instrument_list: List[Dict[str, Union[str, int]]], ticker: str, buy_sell: str, prices: List[float], quantity: int, exchange: str = "NSE") -> Optional[Dict[str, Union[str, int]]]:
        """Place a robo order."""
    ...
    def hist_data_0920(
            self, tickers: List[str], duration: int,
            interval: str, instrument_list: List[Dict[str, Union[str, int]]],
        ...
        for ticker in tickers:
            ...
            try:
                ...
                df_data: pd.DataFrame = pd.DataFrame(hist_data["data"],
                                                     columns=["date", "open", "high", "low", "close", "volume"])
                df_data.set_index("date", inplace=True)
                df_data.index = pd.to_datetime(df_data.index)
                df_data.index = df_data.index.tz_localize(None)
                df_data["gap"] = (
                    (df_data["open"]/df_data["close"].shift(1)) - 1) * 100
